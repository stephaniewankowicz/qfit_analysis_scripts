...
merged_order_all = merged_order_all[merged_order_all['Apo'] =='3k7k']
merged_order_all['Difference'] = merged_order_all['s2calc_x'] - merged_order_all['s2calc_y']
print(merged_order_all.head())

for i in merged_order_all['Holo']:
     print(i)
...
merged_order_all.to_csv('/Users/stephaniewankowicz/Downloads/qfit_paper/Ca2_merge.csv', index=False)

fig = plt.figure()
f, ax = plt.subplots(figsize=(20, 15))
#piv = pd.pivot_table(merged_order_all, values=["Difference"], index=['Ligand'], columns=['res'])
piv =  merged_order_all.pivot("res", "PDB_lig", "Difference")
#print(piv.head())
...
f, ax = plt.subplots(figsize=(20, 15))
#piv_AA = pd.pivot_table(order_CDK2, values=["resn"], index='PDB', columns='resi')
ax = sns.clustermap(piv, yticklabels=True) #, xticklabels=2, yticklabels=2#row_cluster=False,
plt.xticks(rotation=45)
ax.savefig('/Users/stephaniewankowicz/Downloads/qfit_paper/ca2_difference_clustermap.png')
#plt.show()
